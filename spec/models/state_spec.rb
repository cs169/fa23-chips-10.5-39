...
RSpec.describe State, type: :model do
  describe 'associations' do
    it 'has many counties' do
      state = described_class.create!(name: 'Sample State', symbol: 'SS', fips_code: '6', is_territory: false)
  ...
  describe 'std_fips_code' do
    it 'returns FIPS code with leading zeros for single digit code' do
      state = described_class.create!(name: 'Test State', symbol: 'TS', fips_code: '6', is_territory: false)
      expect(state.std_fips_code).to eq('06')
    end

    it 'returns FIPS code as is for two digit code' do
      state = described_class.create!(name: 'Another State', symbol: 'AS', fips_code: '12', is_territory: false)
